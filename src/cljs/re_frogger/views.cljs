...
(defn player
  ...
  []
  (let [x (subscribe [:pos-x])
        ax (anim/interpolate-to x {:duration 333})
        y (subscribe [:pos-y])
        ay (anim/interpolate-to y {:duration 333})]
    (fn render-player []
    [:div {:id "player"
           :style {:top (* @ay 22)
                   :left (* @ax 22)}}])))
...
(defn car
  ...
  [{:keys [x y s]}]
  (let [rx (reagent/atom x)
        ax (anim/interpolate-to rx {:duration (* 333 s)})]
    (fn render-car [{:keys [x y s]}]
      (reset! rx x)
      [:div {:class (str "car model" s)
             :style {:top (* y 22)
                     :left (* @ax 22)}}])))
